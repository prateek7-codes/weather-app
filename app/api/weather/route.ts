...
const toDayKey = (ts: number) => new Date(ts * 1000).toISOString().slice(0, 10);
...
export async function GET(req: NextRequest) {
  ...
  if (!key) return NextResponse.json({ message: 'Missing OPENWEATHER_API_KEY' }, { status: 500 });
  ...
  const [weatherRes, forecastRes] = await Promise.all([
    fetch(`${BASE}/weather?${query}&units=metric&appid=${key}`, { next: { revalidate: 180 } }),
    fetch(`${BASE}/forecast?${query}&units=metric&appid=${key}`, { next: { revalidate: 180 } })
  ]);
  ...
  const openMeteoRes = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${wLat}&longitude=${wLon}&hourly=temperature_2m,uv_index,weather_code&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weather_code&timezone=auto&forecast_days=7`,
    { next: { revalidate: 180 } }
  );
  ...
  const hourly = (forecast.list ?? []).slice(0, 8).map((entry: any) => ({
    time: entry.dt_txt,
  ...
  const forecastMap = new Map<string, { min: number; max: number; icon: string; condition: string; pop: number }>();
  ...
  const forecastDays = Array.from(forecastMap.entries())
    ...
    .map(([date, day], index) => ({
      ...
      icon: day.icon,
      condition: openDaily?.weather_code?.[index] != null ? conditionFromCode(openDaily.weather_code[index]) : day.condition,
      tempMin: openDaily?.temperature_2m_min?.[index] ?? day.min,
      tempMax: openDaily?.temperature_2m_max?.[index] ?? day.max,
      precipitationChance: openDaily?.precipitation_probability_max?.[index] ?? day.pop
    }));
  ...
  const airRes = await fetch(
    `${BASE}/air_pollution?lat=${encodeURIComponent(wLat)}&lon=${encodeURIComponent(wLon)}&appid=${key}`,
    { next: { revalidate: 300 } }
  );
